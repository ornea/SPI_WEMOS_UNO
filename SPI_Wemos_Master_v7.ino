 ...
 * Author J.Richards
 * 
 * Desc: Test sketch for SPI connection between a ESP8266 as master and Arduino Uno as slave.
...
1. You have to decide which pin the MASTER (ESP8266) will be used to Select the SPI device. I used D0.
...
bytes is sent out the SPI port the wrong UNO may respond. Un likley but it is worth mentioning.
   ...
   IO expansion.
   One such Escape Room project required 5 x RFID readers, 1 x Reed switch, 1 x Relay and 1 x Audio Module.
   ...
   If I was to do it again I would use an ESP32. Job done, but the plug in for ARDUINO was still in its infancy at the time
...
#define UNO1_ADDR (((uint32_t)'U')<<24)| (((uint32_t)'1')<<16) //Unique address for UNO See Note2
#define cmdUNO1PINREAD (((uint32_t)'R')<<8) | UNO1_ADDR // ASCII R means want want to (R)ead a pin state
#define cmdUNO1PINHIGH (((uint32_t)'H')<<8)|UNO1_ADDR // ASCII H, set a pin (H)igh
#define cmdUNO1PINLOW (((uint32_t)'L')<<8)|UNO1_ADDR // ASCII L, set a Pin (L)ow
#define cmdUNO1PINTOG (((uint32_t)'T')<<8)|UNO1_ADDR // ASCII T, (T)oggle the state of a pin
#define cmdUNO1PINOUT (((uint32_t)'O')<<8)|UNO1_ADDR // ASCII 'O' set pin to (O)utput
#define cmdUNO1PININP (((uint32_t)'I')<<8)|UNO1_ADDR // ASCII 'I' set pin to (I)nput
#define cmdUNO1PINANALHIGH (((uint32_t)'A')<<8)|UNO1_ADDR  //ASCII 'A' get the (H)igh byte for Analog read 
#define cmdUNO1PINANALLOW (((uint32_t)'a')<<8)|UNO1_ADDR   //ASCII 'a' get the (L)ow byte for Analog read
...
void loop()
{
  Serial.println(send_cmd(cmdUNO1PINTOG | 5)); //This sends the toggle pin 5 command to UNO
